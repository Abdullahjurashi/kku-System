...
public class KkuSystem implements FileNames, ReserveTasks,OptionalReserveTasks{
	 ...
	 * @throws Exception
	 */
	public void showCLIMenu() throws Exception {
		
		load(); //load previous data
		Person p = null; LocalDate l; int t; Room r; boolean st; Scanner sc = new Scanner(System.in); String s;
		...
		while(true) {
			...
			case "2":
				System.out.println("Sorry!we work on it");
				break;
	...
	public boolean checkExist(ReserveBlock rb, List<ReserveBlock> rooms) {
	 ...
	 private Serializable parseDateString(String dateString) throws ParseException {
	...
	public List<ReserveBlock> findReservedBlocksBy(Person p) {
        List<ReserveBlock> reservedBlocksByPerson = new ArrayList<>();
        final List<ReserveBlock> allReservedBlocks = null;
        for (ReserveBlock block : allReservedBlocks) {
            if (block.getBy().equals(p)) {
                reservedBlocksByPerson.add(block);
            }
        }

        return reservedBlocksByPerson;
    }
	@Override
